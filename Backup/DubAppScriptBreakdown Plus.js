...
// Estructura de caché para hojas de cálculo
const sheetCache = {
	initialized: false,
	sheets: new Map()
};

// Estructuras de caché para índices de tablas grandes
const eventIndexCache = {
  initialized: false,
  lastUpdated: null,
  keyToRowMap: {} // Mapeo de EventID a número de fila
};

const filesLinesIndexCache = {
  initialized: false,
  lastUpdated: null,
  keyToRowsMap: {} // Mapeo de FileID a arreglo de filas
};

// Constante para modo de prueba
const TEST_MODE = false; // Cambiar a true para habilitar modo de prueba

// Funciones para inicializar y actualizar los índices
function initializeEventIndex() {
  if(verboseFlag || TEST_MODE) {
    console.log('Inicializando índice para DWO-Event');
  }
  
  const sheet = ssActive.getSheetByName("DWO-Event");
  const lastRow = sheet.getLastRow();
  
  if (lastRow <= 1) {
    eventIndexCache.keyToRowMap = {};
    eventIndexCache.initialized = true;
    eventIndexCache.lastUpdated = new Date();
    return;
  }
  ...
  values.forEach((row, index) => {
    if (row[0]) { // Asegurarnos de que el valor no sea vacío
      eventIndexCache.keyToRowMap[row[0]] = index + 2; // +2 por encabezado y base 0
    }
  });
  ...
  if(verboseFlag || TEST_MODE) {
    console.log('Índice DWO-Event inicializado con ' + Object.keys(eventIndexCache.keyToRowMap).length + ' registros');
  }
}

function initializeFilesLinesIndex() {
  if(verboseFlag || TEST_MODE) {
    console.log('Inicializando índice para DWO_FilesLines');
  }
  
  const sheet = ssActive.getSheetByName("DWO_FilesLines");
  const lastRow = sheet.getLastRow();
  
  if (lastRow <= 1) {
    filesLinesIndexCache.keyToRowsMap = {};
    filesLinesIndexCache.initialized = true;
    filesLinesIndexCache.lastUpdated = new Date();
    return;
  }
  ...
  values.forEach((row, index) => {
    const fileId = row[0];
    if (fileId) { // Asegurarnos de que el valor no sea vacío
      if (!filesLinesIndexCache.keyToRowsMap[fileId]) {
        filesLinesIndexCache.keyToRowsMap[fileId] = [];
      }
      filesLinesIndexCache.keyToRowsMap[fileId].push(index + 2); // +2 por encabezado y base 0
    }
  });
  ...
  if(verboseFlag || TEST_MODE) {
    console.log('Índice DWO_FilesLines inicializado con ' + Object.keys(filesLinesIndexCache.keyToRowsMap).length + ' grupos de FileID');
  }
}

// Funciones para buscar por ID
function findEventById(eventId) {
  if (!eventIndexCache.initialized || TEST_MODE) {
    initializeEventIndex();
  }
  
  const rowNum = eventIndexCache.keyToRowMap[eventId];
  if (!rowNum) {
    if(verboseFlag || TEST_MODE) {
      console.log('No se encontró el eventID ' + eventId + ' en el índice');
    }
    return null;
  }
  
  // Solo leemos la fila específica que necesitamos
  const sheet = ssActive.getSheetByName("DWO-Event");
  return {
    row: rowNum,
    data: sheet.getRange(rowNum, 1, 1, sheet.getLastColumn()).getValues()[0]
  };
}

function findFilesLinesByFileId(fileId) {
  if (!filesLinesIndexCache.initialized || TEST_MODE) {
    initializeFilesLinesIndex();
  }
  
  const rowNums = filesLinesIndexCache.keyToRowsMap[fileId];
  if (!rowNums || rowNums.length === 0) {
    if(verboseFlag || TEST_MODE) {
      console.log('No se encontraron registros para el fileId ' + fileId + ' en el índice');
    }
    return [];
  }
  
  // Para optimizar, agrupamos filas consecutivas en rangos
  const sheet = ssActive.getSheetByName("DWO_FilesLines");
  let results = [];
  
  // Agrupamos filas consecutivas
  rowNums.sort((a, b) => a - b); // Aseguramos que estén ordenadas
  let ranges = [];
  let currentRange = { start: rowNums[0], count: 1 };
  ...
  for (let i = 1; i < rowNums.length; i++) {
    if (rowNums[i] === rowNums[i-1] + 1) {
      ...
      currentRange = { start: rowNums[i], count: 1 };
    }
  }
  ranges.push(currentRange);
  
  // Leemos cada rango de una vez
  if(verboseFlag || TEST_MODE) {
    console.log('Leyendo ' + ranges.length + ' rangos para fileId ' + fileId);
  }
  
  ranges.forEach(range => {
    try {
      const values = sheet.getRange(range.start, 1, range.count, sheet.getLastColumn()).getValues();
      results = results.concat(values);
    } catch (error) {
      console.error('Error al leer rango ' + JSON.stringify(range) + ': ' + error);
    }
  });
...
// Función para programar la actualización asincrónica de índices
function scheduleIndexUpdate() {
  // Guardar el timestamp para saber cuándo se solicitó la actualización
  const props = PropertiesService.getScriptProperties();
  props.setProperty('indexUpdateRequested', new Date().toISOString());
  
  // Programar la actualización para ejecutarse en unos segundos
  ScriptApp.newTrigger('asyncUpdateIndices')
    .timeBased()
    .after(5000) // 5 segundos
    .create();
  
  if(verboseFlag || TEST_MODE) {
    console.log('Actualización de índices programada');
  }
}

function asyncUpdateIndices() {
  try {
    if(verboseFlag || TEST_MODE) {
      console.log('Iniciando actualización asincrónica de índices');
    }
    
    // Abrimos la hoja de cálculo activa si no está abierta
    if (!ssActive) {
    ...
    // Limpiamos la propiedad de solicitud
    PropertiesService.getScriptProperties().deleteProperty('indexUpdateRequested');
    
    // Limpiamos el trigger
    const triggers = ScriptApp.getProjectTriggers();
    triggers.forEach(trigger => {
      if (trigger.getHandlerFunction() === 'asyncUpdateIndices') {
    ...
    sendErrorNotification(error, 'asyncUpdateIndices', {
...
// Función para testear el rendimiento de los índices
function testIndexPerformance() {
  if (!TEST_MODE) {
    console.log('El modo de prueba está desactivado. Cambie TEST_MODE a true para ejecutar pruebas.');
    return;
  }
  
  console.log('=== INICIANDO PRUEBAS DE RENDIMIENTO DE ÍNDICES ===');
  
  // Abrir la hoja de cálculo
  ssActive = SpreadsheetApp.openById(sheetID);
  
  // 1. Prueba de inicialización de índices
  console.log('1. Prueba de inicialización de índices');
  
  console.time('Inicializar eventIndexCache');
  initializeEventIndex();
  console.timeEnd('Inicializar eventIndexCache');
  
  console.time('Inicializar filesLinesIndexCache');
  initializeFilesLinesIndex();
  console.timeEnd('Inicializar filesLinesIndexCache');
  
  // 2. Prueba de búsqueda por ID
  console.log('2. Prueba de búsqueda por ID');
  
  // a) Buscar un eventID existente
  const testEventId = Object.keys(eventIndexCache.keyToRowMap)[0];
  console.log('Buscando eventID: ' + testEventId);
  
  console.time('Búsqueda con índice');
  const resultIndex = findEventById(testEventId);
  console.timeEnd('Búsqueda con índice');
  console.log('Resultado con índice: ' + (resultIndex ? 'Encontrado en fila ' + resultIndex.row : 'No encontrado'));
  
  console.time('Búsqueda tradicional');
  const dwoEventSheet = ssActive.getSheetByName("DWO-Event");
  const dwoEventData = dwoEventSheet.getDataRange().getValues();
  let filaEncontrada = -1;
  for (let i = 0; i < dwoEventData.length; i++) {
    if (dwoEventData[i][0] === testEventId) {
      filaEncontrada = i + 1;
      break;
    }
  }
  console.timeEnd('Búsqueda tradicional');
  console.log('Resultado tradicional: ' + (filaEncontrada > 0 ? 'Encontrado en fila ' + filaEncontrada : 'No encontrado'));
  
  // b) Buscar un fileID existente
  const testFileId = Object.keys(filesLinesIndexCache.keyToRowsMap)[0];
  console.log('Buscando fileID: ' + testFileId);
  
  console.time('Búsqueda con índice');
  const resultFiles = findFilesLinesByFileId(testFileId);
  console.timeEnd('Búsqueda con índice');
  console.log('Resultado con índice: ' + (resultFiles.length > 0 ? 'Encontrados ' + resultFiles.length + ' registros' : 'No encontrado'));
  
  console.time('Búsqueda tradicional');
  const dwoFilesLinesSheet = ssActive.getSheetByName("DWO_FilesLines");
  const dwoFilesLinesData = dwoFilesLinesSheet.getDataRange().getValues();
  let filesEncontrados = [];
  for (let i = 0; i < dwoFilesLinesData.length; i++) {
    if (dwoFilesLinesData[i][1] === testFileId) {
      filesEncontrados.push(dwoFilesLinesData[i]);
    }
  }
  console.timeEnd('Búsqueda tradicional');
  console.log('Resultado tradicional: ' + (filesEncontrados.length > 0 ? 'Encontrados ' + filesEncontrados.length + ' registros' : 'No encontrado'));
  
  // 3. Prueba de programación de actualización
  console.log('3. Prueba de programación de actualización');
  console.time('Programar actualización');
  scheduleIndexUpdate();
  console.timeEnd('Programar actualización');
...
function call() {
	ScriptUpload(
		"Disney+",
		"c11f2a6c-B8LmvCefXVL3he8Jc5puNa3akhBVq4",
		"59208c40",
		"appsheet@mediaaccesscompany.com",
		"F2a8b9a94",
		"Disney+ - David Blaine Do Not Attempt - S1 - Trailer - Teaser (Series) Trailer"
	);
	}
...
// Inicializar índices si aún no están inicializados
if (!eventIndexCache.initialized) {
  initializeEventIndex();
}
if (!filesLinesIndexCache.initialized) {
  initializeFilesLinesIndex();
}
...
// En lugar de retornar statusAux, grabarlo en DWO-Event usando el nuevo sistema de índices
// Buscar la fila con el eventID usando el índice
const eventResult = findEventById(eventID);
var dwoEventSheet = ssActive.getSheetByName("DWO-Event");

if (eventResult) {
    // Columna BC es la número 55 (considerando que A es 1, BC sería 55)
    dwoEventSheet.getRange(eventResult.row, 55).setValue(statusAux);
    
    if(verboseFlag){
        console.log("Estado actualizado en DWO-Event para eventID: " + eventID + " con valor: " + statusAux + " en fila " + eventResult.row);
    }
    
    // Programar actualización asincrónica del índice
    scheduleIndexUpdate();
} else {
...
// Script - Usar el índice para obtener líneas por fileID
// Inicializar el índice si no está inicializado
if (!filesLinesIndexCache.initialized) {
    initializeFilesLinesIndex();
}
...
var dataLines;
if (TEST_MODE) {
    // En modo prueba, comparamos resultados
    console.time('Obtener líneas con índice');
    var linesWithIndex = findFilesLinesByFileId(file_ID);
    console.timeEnd('Obtener líneas con índice');
    
    console.time('Obtener líneas tradicional');
    OpenSheet("DWO_FilesLines", 2, file_ID, 0, ssActive);
    var linesTraditional = auxFilteredValues;
    console.timeEnd('Obtener líneas tradicional');
    
    console.log('Líneas con índice: ' + linesWithIndex.length);
    console.log('Líneas tradicional: ' + linesTraditional.length);
    
    dataLines = linesWithIndex;
} else {
    // En modo normal, usamos directamente el índice
    dataLines = findFilesLinesByFileId(file_ID);
    
    if(verboseFlag) {
        console.log('Líneas obtenidas con índice para file_ID ' + file_ID + ': ' + dataLines.length);
    }
				
    // Si no hay resultados, probamos con el método tradicional como fallback
    if (dataLines.length === 0) {
        if(verboseFlag) {
            console.log('No se encontraron líneas con índice, usando método tradicional');
        }
        OpenSheet("DWO_FilesLines", 2, file_ID, 0, ssActive);
        dataLines = auxFilteredValues;
        
        // Actualizar el índice después de un fallo
        scheduleIndexUpdate();
    }
}
...
// Programar actualización del índice
scheduleIndexUpdate();
}
...
try {
	...
	if (auxFilteredValues && auxFilteredValues.length > 0) {
		...
		if (destinatario && destinatario.trim() !== "") {
			...
			const bcc = "dubapp@mediaaccesscompany.com";
...
function ClearRow(ssAux, keyCol, keyAux) {
  ...
  // Verificar si es una hoja que estamos indexando
  const sheetName = ssAux.getName();
  const isIndexedSheet = (sheetName === "DWO-Event" || sheetName === "DWO_FilesLines");
  var needsIndexUpdate = false;

  // Define las filas que cumplen la condición
  var filasAEliminar = [];
  for (var i = 0; i < datos.length; i++) {
    if (datos[i][keyCol] === keyAux) { 
  ...
  // Si no hay filas que eliminar, salir
  ...
  // Si se eliminaron filas de una hoja indexada, programar actualización
  if (needsIndexUpdate) {
    if(verboseFlag || TEST_MODE) {
      console.log(`Se eliminaron ${filasAEliminar.length} filas de ${sheetName}. Programando actualización de índices.`);
    }
    scheduleIndexUpdate();
  }
}
